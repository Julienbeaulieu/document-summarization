...
class TestEvaluateNewsSummary(unittest.TestCase):
    def test_evaluate(self):
        ...
        t = time()
        predictions, actuals, eval_dict = validate_model(tokenizer, model, device, val_loader, wandb_log=False)  # type: ignore
        time_taken = time() - t

        print(f"rouge1: {eval_dict['rouge1']}, rouge2: {eval_dict['rouge2']}, rougeL: {eval_dict['rougeL']}, time_taken: {time_taken}")
        self.assertGreater(eval_dict['rouge1'], 0.15)
        self.assertGreater(eval_dict['rouge2'], 0.08)
        self.assertGreater(eval_dict['rougeL'], 0.15)
